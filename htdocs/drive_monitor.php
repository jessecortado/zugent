...
//select latest contacts which made a request
...
//Last Active Users
$agents = SQL_QUERY("select u.last_latitude, u.last_longitude, u.first_name, u.last_name, u.last_drive, com.company_name 
	from users as u left join companies as com on u.company_id=com.company_id where (u.last_drive > DATE_SUB(NOW(), INTERVAL 6 HOUR) or u.is_perm_drive = 1) and u.is_active=1");
...
$smarty->assign('agent_requests', $agent_requests);
$smarty->assign('agents_driving', $agents_driving);
$smarty->assign('contacts', $contact_agent_rels);

$smarty->assign('total_agent_requests', SQL_NUM_ROWS($requests));
$smarty->assign('total_agents_driving', SQL_NUM_ROWS($agents));
$smarty->assign('total_contacts', $accepted_cnt);

$smarty->assign('config', $config);

$smarty->assign('footer_js', 'includes/footers/drive_monitor_footer.tpl');

$smarty->display('drive_monitor.tpl');
