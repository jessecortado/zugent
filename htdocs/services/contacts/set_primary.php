...
if(isset($_POST['user_id'])){
	...
	// set all users assigned to contact is_primary=0 before setting a new primary
	...
	$sth = SQL_QUERY("update contacts_rel_users set ".
		"is_primary=".SQL_CLEAN($_POST["set_primary"]).", ".
		"date_updated='".$date."' ".
		"where user_id=".SQL_CLEAN($_POST["user_id"])." and contact_id=".SQL_CLEAN($_POST["contact_id"]));

	// Add contact log (Update)
	add_user_log("Changed the contact's primary user to user_id (".$_POST['contact_file_id'].") from contact_id (".$_POST['contact_id'].")", "contacts", array("importance" => "Info", "action" => "Update") );
