...
if(isset($_POST['lat']) && isset($_POST['lon'])) {
	...
	if ($startpoint['lat'] == $endpoint['lat'] && $startpoint['lng'] == $endpoint['lng']) {
		...
		SQL_QUERY("update users set 
			...
			where user_id='".SQL_CLEAN($_POST['user_id'])."' LIMIT 1");
	}
	else {
	...
	SQL_QUERY("insert into user_locations (latitude,longitude,date_collected,user_id) values ('".SQL_CLEAN($_POST['lat'])."','".SQL_CLEAN($_POST['lon'])."',NOW(),'".SQL_CLEAN($_POST['user_id'])."')");

	echo json_encode(array("error" => false));
	// echo json_encode(array("lat"=>$_POST["lat"], "lon"=>$_POST["lon"], "user_id"=>$_POST["user_id"], "date_collected"=>$date, "direction" => $direction));
