...
if (isset($_POST['latitude']) && isset($_POST['longitude'])) {
	...
	// GOOGLE GEOCODE
	$url = "https://maps.googleapis.com/maps/api/geocode/json?latlng=".$_POST['latitude'].",".$_POST['longitude']."&sensor=true&key=AIzaSyAq8WuTeXonjSzYw_vVLiN7PCRQNqf_v_Q";
	...
	if (!empty($data['administrative_area_level_2 political'])) {
		$state = $data['administrative_area_level_2 political'];
	}
	else {
		$state = "";
	}
	...
	$sth = SQL_QUERY("select user_id, last_latitude, last_longitude, phone_mobile, email, first_name, last_name, is_perm_drive, is_drive_accepted_alert from users 
	...
	if (SQL_NUM_ROWS($sth) == 0) {
		...
		// if($_SERVER['HTTP_HOST'] == "ruopen.com") { // For Live Site
		// 	$host_address = "crm.ruopen.com";
		// }
		// else if($_SERVER['HTTP_HOST'] == "dev.ruopen.com") { // For Dev Site
		// 	$host_address = "https://dev-crm.ruopen.com";
		// }
		// else {
		// 	$host_address = str_replace("ruopen", "zugent", $_SERVER['HTTP_HOST']); // For Localhost
		// 	// $host_address = "https://dev-crm.ruopen.com";
		// }
		if($inArrayURL){
