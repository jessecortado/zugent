...
if(isset($_POST['action']) && $_POST['action'] == 'new_email_template'){
	...
	$sth = SQL_QUERY("insert into user_email_templates (user_id,company_id,template_name,date_created) 
			values (
	...
	$user_email_template_id = SQL_INSERT_ID();

	echo json_encode(array('msg'=>true, 'user_email_template_id'=>$user_email_template_id));
}
else {
	if ($is_ajax) {
