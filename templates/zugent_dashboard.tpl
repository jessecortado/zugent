...
<link href="assets/css/pages/zugent_dashboard.css" rel="stylesheet" />
                ...
                <table id="tbl-companies" class="table table-striped"> 
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Is Billing Current</th>
                            <th class="text-center">Last Charge</th>
                            <th class="text-center">Active Users</th>
                            <th class="text-center">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                    {foreach item=row from=$companies}
                        <tr>
                            <td>{$row.company_id}</td>
                            <td>{$row.company_name}</td>
                            <td class="text-center">{if $row.is_active}<span class="fa fa-check"></span>{/if}</td>
                            <td class="text-center">{if $row.is_billing_current}<span class="fa fa-check"></span>{/if}</td>
                            <td class="text-center">{$row.date_last_charge|date_format}</td>
                            <td class="text-center">{$row.active_users_count}</td>
                            <td class="text-center">
                                <a href="/zugent/company/{$row.company_id}" class="btn btn-success m-r-5 m-b-5 btn-xs"><i class="fa fa-pencil"></i>Edit</a>
                                <a href="/zugent/{$row.company_id}" class="btn btn-success m-b-5 btn-xs"><i class="fa fa-eye"></i>View Users</a>
                            </td>
                        </tr>
                    {/foreach}
                    </tbody>
...
{include file="includes/footer.tpl" footer_js="includes/footers/zugent_dashboard.tpl" company_settings_keyval="1"}