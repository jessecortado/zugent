...
<link href="assets/css/pages/zugent_dashboard_view.css" rel="stylesheet" />

<!-- begin #content -->
<div id="content" class="content">
    <div class="pull-right">
        <a href="/zugent_dashboard.php" class="btn btn-sm btn-danger m-b-5"><i class="fa fa-arrow-left"></i> Back to Zugent Dashboard</a>
    ...
    <h1 class="page-header">{$company.company_name} 
        <ol class="breadcrumb" style="display:inline-block;font-size:12px;">
            <li><a href="javascript:void(0)">Users View</a></li>
                ...
                <table id="acitveUsersTbl" class="table table-striped"> 
                    ...
                    {foreach item=row from=$users}
                        <tr>
                            ...
                            <td class="text-center">{if $row.is_active}<span class="fa fa-check"></span>{else}&nbsp;{/if}</td></td>
                            <td class="text-center">{if $row.is_admin}<span class="fa fa-check"></span>{else}&nbsp;{/if}</td></td>
                            <td class="text-center">{if $row.is_transaction_coordinator}<span class="fa fa-check"></span>{else}&nbsp;{/if}</td></td>
                            <td class="text-center">
                                <a href="/user_view.php?id={$row.user_id}" class="btn btn-success m-r-5 m-b-5 btn-xs"><i class="fa fa-eye"></i>View User</a>
                             </td>
                        </tr>
                    {/foreach}
                    </tbody>
